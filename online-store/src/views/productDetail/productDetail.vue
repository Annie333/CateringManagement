...
import { getGoodsDetail, getFav, addFav, delFav, addShopCart,getShopCart } from '../../api/api';
  export default {
    ...
    created () {
        ...
        console.log('productDetail -> getDetails()')
        this.getDetails();
    },
    watch: {
    ...
    methods: {
        getDetails () { //  请求商品详情
            var t = this
            getGoodsDetail(t.productId)
                .then((response)=> {
                    // console.log(response.data);
                    t.proDetail = response.data;
                    // console.log(t.proDetail)
                    t.curShow = t.proDetail.images[0];
                    console.log(t.curShow)
                }).catch(function (error) {
                    console.log(error);
                });
        },
