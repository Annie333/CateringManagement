...
<script>
  import {getMessages, addMessage, delMessages, getWindows, getStaffMessage, getStaff} from '../../api/api'
    export default {
        data () {
            return {
                message_type: '', // 留言类型
        ...
        methods: {
            ...
            getMessage () { //获取留言
                getStaff({}).then((response)=> {
                        this.windowsId = response.data;
                    }).catch(function (error) {
                        console.log(error);
                    });
                getStaffMessage({
                        windowsId: this.windowsId, //价格最低 默认为‘’ 即为不选价格区间
                    }).then((response)=> {
